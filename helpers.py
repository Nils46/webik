...
# grinder function if category is car
def grinder0():  # if category == "Cars":
    ...
    url_choice = send_url["cars"]
    send_url_2 = (random.choice(url))
    url_choice_2 = send_url_2["cars"]
...
def draw_table():
    ...
    amountdict = db.execute("SELECT amount FROM likes")
    amount1 = []

    for a in amountdict:
        amount1.append(a["amount"])
    amount1 = sorted(amount1, reverse=True)
    amount = amount1[0]

    picurldict = db.execute("SELECT link FROM likes WHERE amount=:amount", amount=amount)
    ...
    return render_template("top.html", likes=amount, data=data, picurl=picurl, user_id=ide, firstname=firstname, surname=surname)
